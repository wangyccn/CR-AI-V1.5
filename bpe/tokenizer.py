...
class BpeTokenizer:
    def __init__(self, path: Optional[str] = None, max_length: int = 2048):
        ...
        try:
            if path and os.path.exists(path):
                ...
                self._tokenizer = Tokenizer(BPE(unk_token=self.unk_token))  # 修改这行
                # 先添加特殊token
    ...
    def encode(self, text: Union[str, List[str]], out_ids: bool = True, padding: bool = True) -> Union[List[int], List[str], List[List[int]], List[List[str]]]:
        """
        增强的编码方法，支持单个文本和文本列表
        Args:
            text: 输入文本或文本列表
    ...
    def _encode_single(self, text: str, out_ids: bool = True, padding: bool = True) -> Union[List[int], List[str]]:
        """单个文本的编码实现"""
        ...
        try:
            ...
            if not encoded.ids and not encoded.tokens:
                ...
                return self._pad_sequence([], out_ids) if padding else ([] if out_ids else [])
    ...
    def _pad_sequence(self, sequence: Union[List[int], List[str]], out_ids: bool) -> Union[List[int], List[str]]:
        """填充序列到指定长度"""
        pad_token = self._tokenizer.token_to_id(self.pad_token) if out_ids else self.pad_token
    ...
    def train_from_iterator(self, iterator: Iterable[str], vocab_size: int, min_freq: int = 2) -> None:
        ...
        try:
            ...
            if vocab_size <= len(self._special_tokens):
                raise ValueError(f"vocab_size必须大于特殊token数量({len(self._special_tokens)})")
