    ...
    def forward(self, x, media_features=None):
        if media_features is not None:
            ...
            if media_features.size(-1) != x.size(-1):
                media_features = nn.Linear(
                    media_features.size(-1), x.size(-1))(media_features)
...
class MegaLLM(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.layers = nn.ModuleList([self._create_layer(
            config, i) for i in range(getattr(config, 'num_layers', 12))])
    ...
    def forward(self, text_ids, images=None, **kwargs):
        ...
        if images is not None:
            ...
            # 修复：将image_encoder实例化为类成员变量
            if not hasattr(self, 'image_encoder'):
                self.image_encoder = MultiModalEncoder(image_dim=x.size(-1)).to(device)
            images = self.image_encoder(images)  # 输出形状为[B, image_dim]
        ...
        def create_custom_forward(layer):
            def custom_forward(*inputs):
                # 确保所有输入在同一设备上
                inputs = [
                    i.to(device) if isinstance(
                        i, torch.Tensor) else i for i in inputs]
                return layer(*inputs)
        ...
        if self.use_gradient_checkpointing and self.training:
            ...
            for layer in self.layers:
                x = layer(x.to(device), images.to(device)
    ...
    def _update_unfinished_sequences(
            self,
        ...
        return unfinished_sequences.mul(
            next_tokens.tile(
                eos_token_id.shape[0],
                1).ne(
                eos_token_id.unsqueeze(1)).prod(
                dim=0))
    ...
    def stream_generate(self, input_ids, images=None, max_length=100, temperature=1.0,
        ...
        with torch.no_grad():
            ...
            if images is not None:
                if not hasattr(self, 'image_encoder'):
                    self.image_encoder = MultiModalEncoder(image_dim=self.embed.embedding_dim)
            ...
            for _ in range(max_length - input_ids.size(1)):
                ...
                # 修复：使用tokenizer实例的eos_token_id属性
                if not hasattr(tokenizer, 'eos_token_id'):
                    tokenizer = tokenizer.BpeTokenizer()  # 实例化tokenizer
...
def test_model():
    ...
    try:
        ...
        text_ids = torch.randint(
            0, config.vocab_size, (2, config.max_seq_len)).to(device)
